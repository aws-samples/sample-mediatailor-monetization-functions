# OpenAds: Video Ad via Ad Server, with Asset Metadata + LiveRamp (EU/UK)
...
aws mediatailor put-playback-configuration \
  ...
  --function-mapping PRE_ADS_REQUEST=ttd-openads-video-asset-eu
```
...
2. **Configurations** → your configuration → **Function mapping** → attach
   `ttd-openads-video-asset-eu` to the **Ad request hook**.
...
of source control, and validating/decoding `asset.*` values (their decoding by
MediaTailor is unverified for these samples).
