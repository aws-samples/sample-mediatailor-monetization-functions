# OpenAds: Video Ad via Ad Server, with Gracenote + LiveRamp (US)
...
aws mediatailor put-playback-configuration \
  ...
  --function-mapping PRE_ADS_REQUEST=ttd-openads-video-gracenote-us
```
...
2. **Configurations** → your configuration → **Function mapping** → attach
   `ttd-openads-video-gracenote-us` to the **Ad request hook**.
...
control, and honoring `coppa`/`dnt`/`lmt`.
