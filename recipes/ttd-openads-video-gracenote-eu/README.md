# OpenAds: Video Ad via Ad Server, with Gracenote + LiveRamp (EU/UK)

Solicit bids for a video ad break from [The Trade Desk](https://www.thetradedesk.com/)
OpenAds, using server-side VAST caching. This variant targets **EU/UK (GDPR)**
traffic and sources content metadata from Gracenote metadata (`player_params.content_*`).

- **Use case:** Ad decisioning
- **Service:** The Trade Desk · HTTP Request
- **Lifecycle hook:** `PRE_ADS_REQUEST`
- **Function type:** `HTTP_REQUEST`
- **Region / privacy:** EU/UK (GDPR)
- **Content source:** Gracenote metadata (`player_params.content_*`)
...
- Content metadata must be present in `player_params.content_*`. Deploy a Gracenote lookup recipe ([by TMS ID](../gracenote-lookup-by-tms-id/) or [by title](../gracenote-lookup-by-title/)) ahead of this one to populate them.
...
| `YOUR_CONSENT_STRING_HERE` | Your TCF v2 consent string from your CMP (GDPR) |
...
aws mediatailor put-playback-configuration \
  ...
  --function-mapping PRE_ADS_REQUEST=ttd-openads-video-gracenote-eu
```
...
2. **Configurations** → your configuration → **Function mapping** → attach
   `ttd-openads-video-gracenote-eu` to the **Ad request hook**.
...
2. A Gracenote lookup ([TMS ID](../gracenote-lookup-by-tms-id/) or [title](../gracenote-lookup-by-title/)) at `PRE_ADS_REQUEST` (content metadata).
...
LiveRamp envelope) and privacy signals to a third-party bidding API. Before
production, review
[docs/security-considerations.md](../../docs/security-considerations.md) — in
particular EU/UK consent obligations (you must supply a valid TCF consent string
from your CMP; without it, DSPs will not bid), keeping your TTD credentials out
of source control, and honoring `coppa`/`dnt`/`lmt`.
